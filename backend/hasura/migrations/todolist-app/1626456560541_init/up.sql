...
CREATE TRIGGER set_public_todolist_updated_at BEFORE UPDATE ON public.todolist FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
COMMENT ON TRIGGER set_public_todolist_updated_at ON public.todolist IS 'trigger to set value of column "updated_at" to current timestamp on row update';
